#!/usr/bin/python3
"""Generate Trickle DML queries for the FirePower benchmark.
...
S3_PREFIX = (
    "s3://firebolt-benchmarks-requester-pays-{{region}}/firenewt/100gb/uservisits/"
)
...
class BenchmarkWriter(ABC):
    ...
    def _make_insert_query(self, table_name, rows):
        ...
        query += ",\n".join(
            [
                f"{spaces}('{r[0]}', '{escape_string(r[1])}', '{r[2]}', {r[3]}, '{r[4]}', '{r[5]}', '{r[6]}', '{escape_string(r[7])}', {r[8]})"
                for r in rows
            ]
        )
...
class FirePowerBenchmarkWriter(BenchmarkWriter):
    """Implementation of BenchmarkWriter for writing the scenario in the FirePower format."""

    DIR = os.path.join(current_dir, "..", "SQL", "trickle_ingestion")
    WORKLOAD_REPLAY_INSERT_FILENAME_TEMPLATE = (
        "insert_{rows_per_query}r_{num_queries}q.csv"
    )
    WORKLOAD_REPLAY_UPDATE_FILENAME_TEMPLATE = (
        "update_{rows_per_query}r_{num_queries}q.csv"
    )
    WORKLOAD_REPLAY_DELETE_FILENAME_TEMPLATE = (
        "delete_{rows_per_query}r_{num_queries}q.csv"
    )
    ...
    def _write_scenario(self, filename, table_name, query_id_prefix, query_generator):
        ...
        write_row(
            "system_prep_1_drop_ext", f"DROP TABLE IF EXISTS external_{table_name};"
        )
        write_row(
            "system_prep_2_create_ext",
        ...
        write_row(
            "system_prep_5_ingest",
            f"INSERT INTO {table_name} SELECT * FROM external_{table_name};",
        )
        ...
        write_row(
            "system_cleanup_2_drop_et", f"DROP TABLE IF EXISTS external_{table_name};"
        )

        df = pd.DataFrame(
            csv_rows,
            columns=["query_start_ts", "query_end_ts", "query_id", "query_text"],
        )
        df.to_csv(path, index=False, quoting=csv.QUOTE_NONNUMERIC)

    def _write_insert_scenario(
        self, name, rows_per_query, num_queries, rows_for_insert_generator
    ):
        ...
        def query_generator():
            ...
            for _ in range(num_queries):
                query = self._make_insert_query(
                    table_name=table_name,
                    rows=[
                        next(rows_for_insert_generator) for _ in range(rows_per_query)
                    ],
                )
                yield query

        self._write_scenario(
            filename=self.WORKLOAD_REPLAY_INSERT_FILENAME_TEMPLATE.format(
                rows_per_query=rows_per_query, num_queries=num_queries
            ),
            table_name=table_name,
    ...
    def _write_update_scenario(
        self, name, rows_per_query, num_queries, target_row_generator
    ):
        ...
        def query_generator():
            ...
            for i in range(num_queries):
                query = self._make_update_query(
                    table_name=table_name, row=next(target_row_generator)
                )
                yield query

        self._write_scenario(
            filename=self.WORKLOAD_REPLAY_UPDATE_FILENAME_TEMPLATE.format(
                rows_per_query=rows_per_query, num_queries=num_queries
            ),
            table_name=table_name,
    ...
    def _write_delete_scenario(
        ...
        def query_generator():
            ...
            for _ in range(num_queries):
                query = self._make_delete_query(
                    table_name=table_name, row=next(target_row_generator)
                )
                yield query

        self._write_scenario(
            filename=self.WORKLOAD_REPLAY_DELETE_FILENAME_TEMPLATE.format(
                rows_per_query=rows_per_query, num_queries=num_queries
            ),
            table_name=table_name,
...
def get_randomly_chosen_row_from_table_generator(cursor, batch_size):
    ...
    while True:
        ...
        if not row_pool:
            ...
            cursor.execute(
                f"SELECT * FROM {SOURCE_TABLE} WHERE random() < CAST({batch_size * 10} AS FLOAT) / (SELECT count(*) FROM {SOURCE_TABLE});"
            )
...
def generate_queries():
    ...
    w = FirePowerBenchmarkWriter()

    print("Creating INSERT scenarios...\n")

    num_rows_to_generate_for_inserts = sum(
        [
            params["rows_per_query"] * params["num_queries"]
            for params in INSERT_BENCHMARK_SCENARIOS.values()
        ]
    )
    ...
    if len(rows_for_inserts) != num_rows_to_generate_for_inserts:
        print(
            f"Expected {num_rows_to_generate_for_inserts} rows, but got {len(rows_for_inserts)} rows. "
    ...
    num_random_rows = sum(
        [params["num_queries"] for params in UPDATE_BENCHMARK_SCENARIOS.values()]
    ) + sum([params["num_queries"] for params in DELETE_BENCHMARK_SCENARIOS.values()])
    random_row_chooser = get_randomly_chosen_row_from_table_generator(
        cursor, num_random_rows
    )
